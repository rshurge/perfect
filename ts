function GetUi() 
    if getgenv().Tvk then
    if game.CoreGui:FindFirstChild("Skidded Hub GUI") then
        for a, v in ipairs(game.CoreGui:GetChildren()) do
            if v.Name == "Skidded Hub GUI" then
                v:Destroy()
            end
        end
    end
...
function SaveCustomUISettings()
    local j = game:GetService("HttpService")
    if not isfolder("Skidded Hub") then
        makefolder("Skidded Hub")
    end
    writefile("Skidded Hub/" .. k, j:JSONEncode(CorrectTable(e)))
end
function ReadCustomUISetting()
    local l, m =
        ...
        function()
            ...
            local n = j:JSONDecode(readfile("Skidded Hub/" .. k))
...
u.NotiGui.Name = "Skidded Hub Notification"
...
function t.CreateNoti(H)
    ...
    local g = V .. "," .. W .. "," .. X
    Q.Text = '<font color="rgb(' .. g .. ')">Skidded Hub</font> ' .. getgenv().TitleNameNoti
    table.insert(
        c["Title Text Color"],
        function()
            local V = tostring(u.Getcolor(getgenv().UIColor["Title Text Color"])[1])
...
function t.CreateMain(H)
    local Z = tostring(H.Title) or "Skidded Hub"
    ...
    function u.ReloadMain(v)
        ...
        local X = tostring(u.Getcolor(getgenv().UIColor["Title Text Color"])[3])
        local g = V .. "," .. W .. "," .. X
        ad.Text = '<font color="rgb(' .. g .. ')">Skidded Hub</font> ' .. getgenv().MainDesc
        table.insert(
            c["Title Text Color"],
            function()
                aa.ImageColor3 = getgenv().UIColor["Title Text Color"]
...
function SaveSettings()
    ...
    writefile("Skidded Hub/" .. SaveFileName, HttpService:JSONEncode(Settings))
end
...
function ReadSetting() 
    local s,e = pcall(function() 
        ...
        return HttpService:JSONDecode(readfile("Skidded Hub/" .. SaveFileName))
    end)
    if s then return e 
    else
        SaveSettings()
...
local Folder2 = game:GetService("Workspace").GlitchParticlesFolder
...
function Anti1() 
    ...
    for i, v in pairs(GetListField()) do
        if v ~= "Skidded Hub Winning Field" then
...
GUISettingSection.CreateBind({Title = 'Toggle GUI', Key = Enum.KeyCode.RightControl}, function()
    ...
    if game.CoreGui:FindFirstChild("Skidded Hub GUI")then for a,b in ipairs(game.CoreGui:GetChildren())do if b.Name=="Skidded Hub GUI"then b.Enabled=getgenv().GuiToggled end end end
end)
if getgenv().Key == nil then
    GUISettingSection.CreateButton({Title = 'Destroy GUI'}, function()
	    if game.CoreGui:FindFirstChild("Skidded Hub GUI")then for a,b in ipairs(game.CoreGui:GetChildren())do if b.Name=="Skidded Hub GUI"then b:Destroy()end end end
    end)
end
-- UI LIB --
            ...
            if Settings.Farm and HumanoidRootPart then
                ...
                if token then
                    if getgenv().IsValidTokenPos(token, TempField,FieldPosIn) and getgenv().IsToken(token) and not getgenv().IsTokenBlacklist(token) and Settings.Farm and ValidFarm() then
                     ...
                     if TempField ~= "Skidded Hub Winning Field" and plr.Character:FindFirstChild("HumanoidRootPart") then 
                    ...
                    embeds = {
                        {
                            ...
                            author = {
                                name = "Skidded Hub Bee Swarm Simulator Bug Reporter"
                            }
                        }
                    }
                }
